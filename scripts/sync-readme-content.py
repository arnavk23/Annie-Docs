...
def extract_sections(readme_content):
    ...
    usage_match = re.search(r'## (?:Usage|Basic Usage|Quick Start)\s*\n(.*?)(?=\n##|\Z)', readme_content, re.DOTALL)
    if usage_match:
        sections['usage'] = usage_match.group(1).strip()
    
    # Extract features
    features_match = re.search(r'## (?:Features|Key Features)\s*\n(.*?)(?=\n##|\Z)', readme_content, re.DOTALL)
    if features_match:
        sections['features'] = features_match.group(1).strip()
    
    # Extract performance benchmarks
    perf_match = re.search(r'## (?:Performance|Benchmarks?)\s*\n(.*?)(?=\n##|\Z)', readme_content, re.DOTALL)
    if perf_match:
